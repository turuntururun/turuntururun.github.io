...
export const scaleComposer = (
  base: (root: number) => number[],
  ...modifiers: ((source: number[]) => number[])[]
) => {
  return (root: number) => {
...
export const onDegree = (degree: number) => (source: number[]) => {
  ...
  return [
    ...source.slice(degree - 1),
    ...source.slice(0, degree - 1).map((a) => a + 12),
  ]
}
...
export const major = (root: number) => [
  root,
  root + 2,
  root + 4,
  root + 5,
...
export const generateFingering = (
  notes: number[],
  tuning: { string: number; note: number }[] = standardE,
) => {
  ...
  notes.forEach((note) => {
    ...
    if (fret >= baseFret + fretCoverage) {
      ...
      result.push({ string: tuningElement.string, fret: fret })
    } else {
      result.push({ string: tuningElement.string, fret: fret })
    }
  })
...
export const parseChord = (notes: number[]): string[] => {
  ...
  let flavor = ''
  if (third == 4) {
    if (fifth == 7) flavor = 'major'
    if (fifth == 8) flavor = 'aug'
  } else if (third == 3) {
    if (fifth == 7) flavor = 'minor'
    if (fifth == 6) flavor = 'dim'
  }
